	...
	const cctv = {
		...
		grid: {
				   ...
				   /* oColNames.push('상시');
					oColModel.push({
						...
						formatter: function (cellvalue, options, rowObject) {
							...
							if (rowObject.ivaTy.charAt(2) == '2') sHtml = '<i class="fas fa-check-square"></i>';
							const $btn = $('<button/>', {
								'type': 'button',
								'class': 'btn btn-default btn-xs btn-update',
								'title': '상시',
			...
			params: function (id) {
				...
				if (id === 'cctv-list') {
					//oParams.cpltPrdtTy = 'N';
					oParams.recordingYn = 'N';
				//} else if (id === 'trgt-cctv-list' && $('#chk-always-y').is(':checked')) {
				} else if (id === 'trgt-cctv-list') {
					//oParams.cpltPrdtTy = 'Y';
