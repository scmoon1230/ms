...
//
function HiVeWebRtcPlayer(element, server_ip, server_port, video_name, use_single_port, use_playback, stime, etime, speed) {
    "use strict";

    if (use_single_port === undefined) use_single_port = true;
    if (use_playback === undefined) use_playback = false;
    if (stime === undefined) stime = 0;
    if (etime === undefined) etime = 0;
    ...
    var video_module = element;
    var show_controls = video_module.controls;

    let interval = null;

    this.videoElement = video_module;
    ...
    this.Play = function Play() {
        ...
        if ((state === "ready") && (cur_time - latest_stop_time > 1000)) {
            ...
            state = "start";
            is_connected = false;
            video_module.setAttribute('style', 'background: black url(/js/mntr/vms/HIVE/loader.gif) center no-repeat;');

            show_controls = video_module.controls;
            video_module.controls = false;

            if (speed > 1) video_module.playbackRate = 1.0 * speed;

            DoSignaling();
        }

        if (cur_time - latest_stop_time <= 1000) video_module.pause();
    };
    ...
    this.DoPtzControl = function DoPtzControl(cmd, action, direction, speed, num, continuous) {
        ...
        if (cmd === null || action === null || video_name === null) return false;
        ...
        ws.onmessage = function (evt) {
            ...
            if (data.result !== 200) console.error("Request Failed:", data.message);
        };
    ...
    this.GetPtzSpeed = function GetPtzSpeed() {
        return new Promise(function (resolve, reject) {
            ...
            ws.onmessage = function (evt) {
                ...
                if (data.result !== 200) reject("Request Failed: ", data.message);
                else resolve(data.speed);
    ...
    function gotRemoteStream(e) {
        ...
        if (show_controls) video_module.controls = true;
    }
    ...
    function Terminate() {
        ...
        /*
        if (state !== "stop") video_module.setAttribute('style', 'background:black url(/js/mntr/vms/HIVE/loader.gif) center no-repeat;');
        else video_module.setAttribute('style', 'background-color:black');
        */
        video_module.load();
        ...
        video_module.setAttribute('style', 'background:#000 url(/js/mntr/vms/HIVE/eov.png) 50% center no-repeat;');
    }
        ...
        ws.onmessage = function (evt) {
            ...
            if (json_obj.result === 200) {
                if (json_obj.type === "join") {
                    ...
                    pc.createOffer(offerOptions).then(onCreateOfferSuccess, function () {
                        Terminate();
                    ...
                    var candidate = new RTCIceCandidate({
                        sdpMLineIndex: server_endpoint.sdpMLineIndex,
                        candidate: server_endpoint.candidate
                    });
