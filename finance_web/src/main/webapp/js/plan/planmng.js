...
$(document).ready(function(){
	...
	$.jqGrid($('#grid'), {
		url:contextRoot + "/plan/mng/list.json",
		datatype: "json",
		autowidth: true,
		postData: {
			stanYy         : $("#stanYy").val()
		...
		colModel: [{ name: 'CHECK', width:50 , align:'center', editable:true, edittype:'checkbox' , hidden:true
					...
					}, { name: 'planAmt'        , index: 'PLAN_AMT'        , width:80 , align:'right'
						, cellattr : function() {	return 'style="padding-right: 5px;"';	}
						, formatter : function(cellvalue, options, rowObject) {
							return rowObject.planAmt.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
						}
					}
		  		],
		pager: '#pager',
		...
		sortname: 'MAP.ACCT_GB, MAP.ACCT_CODE',
		sortorder: 'ASC',
		viewrecords: true,
		multiselect: false,
		loadonce:false,
...
function insertAction(obj) {
	...
	var url = contextRoot + "/plan/mng/insert.json";
...
function updateAction(obj) {
	var url = contextRoot + "/plan/mng/update.json";
	var params = "memberId=" + encodeURIComponent($("#iMemberId").val().trim());
		params += "&memberNo=" + encodeURIComponent($("#iMemberNo").val().trim());
		params += "&memberName=" + encodeURIComponent($("#iMemberName").val().trim());
		params += "&sexType=" + encodeURIComponent($("#iSexType").val());
...
function deleteAction(obj) {
	var url = contextRoot + "/plan/mng/delete.json";
...
function deleteMultiAction(obj) {
	...
	var url = contextRoot + "/plan/mng/deleteMulti.json";
	var params = "";
