...
<script>
	...
	var sortBy = '';
	if ( '1' == '${type}' ) {
		sortBy = 'STAN_YMD, MONEY_CODE, MEMBER_ID';
	} else if ( '2' == '${type}' ) {
		sortBy = 'STAN_YMD, MONEY_CODE ASC, MEMBER_ID DESC, MEMBER_NAME';
//	} else if ( '3' == '${type}' ) {
//		sortBy = 'STAN_YMD, MONEY_CODE, MONEY_AMT';
	}
</script>
<script src="<c:url value='/js/report/rprtmoney.js'/>"></script>
</head>
<body>
					...
					<table>
						...
						<tr>
							...
							<td><select name="" id="deptCode" class="selectType1" maxlength="1" onchange="$('.btnS').click()">
								...
								/
								<select name="" id="regionCode" class="selectType1" maxlength="1" onchange="$('.btnS').click()">
									<option value="">전체</option>
							...
							<th>아이디여부</th>
							<td><select name="" id="memberNoFlag" class="selectType1" maxlength="1" onchange="$('.btnS').click()">
									<option value="">전체</option>
									<option value="Y" ${'1' == type ? 'selected' : ''} >ID가 있는 헌금만 검색</option>
									<option value="N">ID가 없는 헌금만 검색</option>
								</select>
								<a href="javascript:;" class="btn btnRight btnS searchBtn">검색</a>
							</td>
						<%--<th>기준금액</th>
							<td><select name="" id="moneyAmt123" class="selectType1" maxlength="1" onchange="$('.btnS').click()">
									<option value="">전체</option>
