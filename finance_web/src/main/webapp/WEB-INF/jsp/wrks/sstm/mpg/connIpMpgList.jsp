...
$(document).ready(function(){
	...
	$.jqGrid($('#grid'), {
		  url: '<c:url value='/'/>wrks/sstm/mpg/connIpMpgListData.json'
		, datatype: "json"
		, postData: {
			  connIp : $("#connIP").val()
		...
		, colNames: ['<input type="checkbox" name="hcheckbox" onchange="$.GridHeaderCheckBoxChange(\'#grid\', this, event);">'
					, 'No',		'접속IP(사용자PC IP)',		'네트워크아이디',		'비고',		'수정일'
		]
...
function updateAction(obj) {
	//alert('updateAction');
	var url = "<c:url value='/'/>wrks/sstm/mpg/connIpMpgList/update.json";
	var params = "connIp=" + $("#iConnIp").val();
	params += "&networkId=" + $("#iNetworkId").val();
...
function insertAction(obj)
{
	if ( ipCheck())
	{
		var url = "<c:url value='/'/>wrks/sstm/mpg/connIpMpgList/insert.json";
		var params = "&connIp=" + $("#iConnIp").val();
		params += "&networkId=" + $("#iNetworkId").val();
		params += "&connDesc=" + $("#iConnDesc").val();
...
function deleteAction(obj) {
	...
	var url = "<c:url value='/'/>wrks/sstm/mpg/connIpMpgList/delete.json";
	var params = "connIp=" + $("#iConnIp").val();
...
function deleteMultiAction(obj) {
	...
	var url = "<c:url value='/'/>wrks/sstm/mpg/connIpMpgList/deleteMulti.json";
	var params = "";
	...
	<div class="container">
		...
		<div class="contentWrap">
			...
			<div class="layer layerDetail" id="div_drag_1">
				<div class="tit"><h4>망연계 접속 IP 매핑</h4></div>
				<div class="layerCt">
					<div class="tableType2">
						<table>
							<caption>망연계 기본매핑 정보</caption>
							<tbody>
								<tr><th>접속 IP</th>				<td id="dConnIp"></td>
									<th>네트워크아이디</th>			<td id="dNetworkId"></td>
								</tr>
								<tr><th>비고</th>					<td id="dConnDesc" ></td>
									<th>수정일</th>				<td id="dUpdDate" ></td>
								</tr>
							</tbody>
						</table>
					</div>
			...
			<div class="layer layerRegister" id="div_drag_2">
				<div class="tit"><h4>망연계 접속 IP 매핑 <span id="modetitle">추가</span></h4></div>
				<div class="layerCt">
					<div class="tableType2">
						<table>
							 <caption>망연계 기본매핑 정보</caption>
							<tbody>
								<tr><th><sup><i class="fas fa-asterisk fa-xs text-danger"></i></sup>접속IP</th>
									...
									<th><sup><i class="fas fa-asterisk fa-xs text-danger"></i></sup>네트워크아이디</th>
									<td><input type="text" id="iNetworkId" class="txtType" maxlength="100" required="required" user-required="insert" user-title="네트워크 아이디" /></td>
									<td class="sNetworkId" width="100"><a href="#" class="btn addRpt">네트워크 아이디</a></td>
								</tr>
								<tr><th>비고</th>
									<td colspan ="4">
										<input type="text" id="iConnDesc" class="txtType" maxlength="100" style="width:100%;"/>
									</td>
								</tr>
							</tbody>
						</table>
