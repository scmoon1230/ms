...
<%
	...
	ChartServer chartAmt = new ChartServer(pageContext, request, response);
	...
	for(int i = 0, k = 0; i < troubleGbList.size(); i++) {
		...
		if(((String)troubleGbList.get(i).get("CD_ID")).equals("03")) {
			series[0] = Integer.parseInt(String.valueOf(((JsonObject)jsonArr.get(0)).get("UNPAID_AMT").getAsString()));
			series[1] = Integer.parseInt(String.valueOf(((JsonObject)jsonArr.get(2)).get("UNPAID_AMT").getAsString()));

			lstProvider.add(series);												// 데이타셋 추가
			titleArr.add((String)troubleGbList.get(i).get("CD_NM_KO"));
		}
		else if(((String)troubleGbList.get(i).get("CD_ID")).equals("05")) { //대포
			series[0] = Integer.parseInt(String.valueOf(((JsonObject)jsonArr.get(1)).get("UNPAID_AMT").getAsString()));
			series[1] = Integer.parseInt(String.valueOf(((JsonObject)jsonArr.get(3)).get("UNPAID_AMT").getAsString()));
	...
	chartAmt.setDataSource(lstProvider);												
	
	
	//System.out.println(labels);
	
	//chartAmt.getData().setSeries(1); 										// 데이타셋 수 설정 (데이타셋에 따라 자동으로 설정, 수동으로 설정 필요시 추가 )
	//chartAmt.getData().setPoints(3); 										// X축 Label 수 설정 (데이타셋에 따라 자동으로 설정, 수동으로 설정 필요시 추가 )
	chartAmt.setGallery(Gallery.BAR); 										// 그래프 타입 : BAR, PIE
	chartAmt.getTitles().add(new TitleDockable("영치 금액"));		//타이틀 설정
	chartAmt.setWidth(295);													// 폭설정
	chartAmt.setHeight(220);													// 높이 설정
	chartAmt.getLegendBox().setVisible(true);									// 범례 유무
	chartAmt.getLegendBox().setDock(DockArea.BOTTOM);							// 범례 위치
	chartAmt.getAxisY().setStep(10000);										// Y축 눈금 단위 설정		

	//System.out.println(labels);
	
	for(int i = 0; i < labels.length; i++) {
		chartAmt.getSeries().get(i).setText((String)labels[i]);								//범례 설정
	}
	
	// Chart 그리기
	chartAmt.renderControl();
%>
</body>
</html>
--%>