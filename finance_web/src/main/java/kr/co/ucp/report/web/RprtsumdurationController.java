...
import kr.co.ucp.report.service.RprtsumdurationService;

@Controller
public class RprtsumdurationController {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());

	@Resource(name="rprtsumdurationService")
	RprtsumdurationService rprtsumdurationService;
	...
	// 리스트
	@RequestMapping(value="/report/sumduration.do")
	public String view(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        String startDay = EgovDateUtil.addYearMonthDay(toDate, 0, -1, 0);
        //System.out.println("startDay = "+startDay);
        startDay = startDay.substring(0,6)+"01";
        //System.out.println("startDay = "+startDay);
        request.setAttribute("startDate", EgovDateUtil.formatDate(startDay, "-"));
		...
		return "report/rprtsumduration";
	}

	// 조건검색
	@RequestMapping(value="/report/sumduration/list.json")
	@ResponseBody
	public Map<String, Object> list(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<Map<String, String>> list = rprtsumdurationService.selectMoney(args);
	...
	@RequestMapping(value="/report/sumduration/getSum.json")
	@ResponseBody
	public Map<String, Object> getSum(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			String totalAmnt = rprtsumdurationService.selectTotalAmnt(args);
	...
	// 등록
	@SuppressWarnings("unused")
	@RequestMapping(value="/report/sumduration/insert.json")
	@ResponseBody
	public Map<String, Object> insert(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int insertResult;
		try {
			insertResult = rprtsumdurationService.insertMoney(map);
			mapRet.put("session", 1);
	...
	// 수정
	@SuppressWarnings("unused")
	@RequestMapping(value="/report/sumduration/update.json")
	@ResponseBody
	public Map<String, Object> update(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int updateResult;
		try {
			updateResult = rprtsumdurationService.updateMoney(map);
			mapRet.put("session", 1);
			mapRet.put("msg", "저장하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
	...
	// 삭제
	@RequestMapping(value="/report/sumduration/delete.json")
	@ResponseBody
	public Map<String, Object> delete(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int result = rprtsumdurationService.deleteMoney(map);
	...
	// 다중삭제
	@SuppressWarnings("unused")
	@RequestMapping(value="/report/sumduration/deleteMulti.json")
	@ResponseBody
	public Map<String, Object> deleteMulti(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int ret;
		try {
			ret = rprtsumdurationService.deleteMoneyMulti(list);
			mapRet.put("session", 1);
			mapRet.put("msg", "삭제하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
