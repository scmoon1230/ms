...
@Controller
public class RprtsummonthController {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());

	@Resource(name="rprtsummonthService")
	RprtsummonthService rprtsummonthService;
	...
	// 리스트
	@RequestMapping(value="/report/summonth.do")
	public String view(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		return "report/rprtsummonth";
	}

	// 조건검색
	@RequestMapping(value="/report/summonth/list.json")
	@ResponseBody
	public Map<String, Object> list(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Map<String, String> durMonth = rprtsummonthService.selectDurationMonth(args);
		Map<String, String> durSumm = rprtsummonthService.selectDurationSumm(args);

		args.put("stanYy"      , stanYmd.substring(0,4));
		List<Map<String, String>> list = moneyService.selectMoney(args);		// 헌금
		for( int j=0; j<list.size() ; j++ ) {
			list.get(j).put("moneyAmtMonth", "0");
			list.get(j).put("moneyAmtSumm", "0");
		}
		
		List<Map<String, String>> listMonth = rprtsummonthService.selectMoneyMonth(args);	// 월계
		if ( listMonth != null ) {
			for ( int i=0 ; i<listMonth.size() ; i++ ) {
				for( int j=0 ; j<list.size() ; j++ ) {
					...
					if(listMonth.get(i).get("moneyCode").toString().equalsIgnoreCase(list.get(j).get("moneyCode").toString())) {
						...
						list.get(j).put("moneyAmtMonth", String.valueOf(listMonth.get(i).get("moneyAmt")));
					}
				}
			}
		}

		List<Map<String, String>> listSumm = rprtsummonthService.selectMoneySumm(args);		// 누계
		if ( listSumm != null ) {
		...
		map.put("durMonth", durMonth.get("duration").toString());
	...
	@RequestMapping(value="/report/summonth/getSum.json")
	@ResponseBody
	public Map<String, Object> getSum(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			String monthAmnt = rprtsummonthService.selectMonthAmnt(args);
			String totalAmnt = rprtsummonthService.selectTotalAmnt(args);
			mapRet.put("session", 1);
			mapRet.put("monthAmnt", monthAmnt);
	...
	// 등록
	@SuppressWarnings("unused")
	@RequestMapping(value="/report/summonth/insert.json")
	@ResponseBody
	public Map<String, Object> insert(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int insertResult;
		try {
			insertResult = rprtsummonthService.insertMoney(map);
			mapRet.put("session", 1);
	...
	// 수정
	@SuppressWarnings("unused")
	@RequestMapping(value="/report/summonth/update.json")
	@ResponseBody
	public Map<String, Object> update(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int updateResult;
		try {
			updateResult = rprtsummonthService.updateMoney(map);
			mapRet.put("session", 1);
			mapRet.put("msg", "저장하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
	...
	// 삭제
	@RequestMapping(value="/report/summonth/delete.json")
	@ResponseBody
	public Map<String, Object> delete(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int result = rprtsummonthService.deleteMoney(map);
	...
	// 다중삭제
	@SuppressWarnings("unused")
	@RequestMapping(value="/report/summonth/deleteMulti.json")
	@ResponseBody
	public Map<String, Object> deleteMulti(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int ret;
		try {
			ret = rprtsummonthService.deleteMoneyMulti(list);
			mapRet.put("session", 1);
			mapRet.put("msg", "삭제하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
