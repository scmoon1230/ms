...
@Controller
public class SheetyeardifferController {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());

	@Resource(name="sheetyeardifferService")
	SheetyeardifferService sheetyeardifferService;
	...
	// 리스트
	@RequestMapping(value="/sheet/yeardiffer.do")
	public String view(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		return "sheet/sheetyeardiffer";
	}

	// 조건검색
	@RequestMapping(value="/sheet/yeardiffer/list.json")
	@ResponseBody
	public Map<String, Object> list(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<Map<String, String>> list = sheetyeardifferService.selectSheetyeardiffer(args);
	...
	@RequestMapping(value="/sheet/yeardiffer/insert.json")
	@ResponseBody
	public Map<String, Object> insert(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int insertResult;
		try {
			insertResult = sheetyeardifferService.insertSheet(map);
			mapRet.put("session", 1);
	...
	// 수정
	@SuppressWarnings("unused")
	@RequestMapping(value="/sheet/yeardiffer/update.json")
	@ResponseBody
	public Map<String, Object> update(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int updateResult;
		try {
			updateResult = sheetyeardifferService.updateSheet(map);
			mapRet.put("session", 1);
			mapRet.put("msg", "저장하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
	...
	// 삭제
	@RequestMapping(value="/sheet/yeardiffer/delete.json")
	@ResponseBody
	public Map<String, Object> delete(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int result = sheetyeardifferService.deleteSheet(map);
	...
	// 다중삭제
	@SuppressWarnings("unused")
	@RequestMapping(value="/sheet/yeardiffer/deleteMulti.json")
	@ResponseBody
	public Map<String, Object> deleteMulti(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int ret;
		try {
			ret = sheetyeardifferService.deleteSheetMulti(list);
			mapRet.put("session", 1);
			mapRet.put("msg", "삭제하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
