...
@Controller
public class SheetcashtotalallController {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());

	@Resource(name="sheetcashtotalallService")
	SheetcashtotalallService sheetcashtotalallService;
	...
	// 리스트
	@RequestMapping(value="/sheet/cashtotalall.do")
	public String view(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		return "sheet/sheetcashtotalall";
	}

	// 조건검색
	@RequestMapping(value="/sheet/cashtotalall/list.json")
	@ResponseBody
	public Map<String, Object> list(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<Map<String, String>> list = sheetcashtotalallService.selectSheetCashtotalall(args);
	...
	@RequestMapping(value="/sheet/cashtotalall/insert.json")
	@ResponseBody
	public Map<String, Object> insert(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int insertResult;
		try {
			insertResult = sheetcashtotalallService.insertSheet(map);
			mapRet.put("session", 1);
	...
	// 수정
	@SuppressWarnings("unused")
	@RequestMapping(value="/sheet/cashtotalall/update.json")
	@ResponseBody
	public Map<String, Object> update(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int updateResult;
		try {
			updateResult = sheetcashtotalallService.updateSheet(map);
			mapRet.put("session", 1);
			mapRet.put("msg", "저장하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
	...
	// 삭제
	@RequestMapping(value="/sheet/cashtotalall/delete.json")
	@ResponseBody
	public Map<String, Object> delete(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int result = sheetcashtotalallService.deleteSheet(map);
	...
	// 다중삭제
	@SuppressWarnings("unused")
	@RequestMapping(value="/sheet/cashtotalall/deleteMulti.json")
	@ResponseBody
	public Map<String, Object> deleteMulti(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int ret;
		try {
			ret = sheetcashtotalallService.deleteSheetMulti(list);
			mapRet.put("session", 1);
			mapRet.put("msg", "삭제하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
