...
@Controller
public class SheetcashtotalController {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());

	@Resource(name="sheetcashtotalService")
	SheetcashtotalService sheetcashtotalService;
	...
	// 리스트
	@RequestMapping(value="/sheet/cashtotal.do")
	public String view(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		return "sheet/sheetcashtotal";
	}

	// 조건검색
	@RequestMapping(value="/sheet/cashtotal/list.json")
	@ResponseBody
	public Map<String, Object> list(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<Map<String, String>> list = sheetcashtotalService.selectSheetCashtotal(args);
	...
	@RequestMapping(value="/sheet/cashtotal/insert.json")
	@ResponseBody
	public Map<String, Object> insert(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int insertResult;
		try {
			insertResult = sheetcashtotalService.insertSheet(map);
			mapRet.put("session", 1);
	...
	// 수정
	@SuppressWarnings("unused")
	@RequestMapping(value="/sheet/cashtotal/update.json")
	@ResponseBody
	public Map<String, Object> update(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int updateResult;
		try {
			updateResult = sheetcashtotalService.updateSheet(map);
			mapRet.put("session", 1);
			mapRet.put("msg", "저장하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
	...
	// 삭제
	@RequestMapping(value="/sheet/cashtotal/delete.json")
	@ResponseBody
	public Map<String, Object> delete(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int result = sheetcashtotalService.deleteSheet(map);
	...
	// 다중삭제
	@SuppressWarnings("unused")
	@RequestMapping(value="/sheet/cashtotal/deleteMulti.json")
	@ResponseBody
	public Map<String, Object> deleteMulti(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int ret;
		try {
			ret = sheetcashtotalService.deleteSheetMulti(list);
			mapRet.put("session", 1);
			mapRet.put("msg", "삭제하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
