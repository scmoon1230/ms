...
@Controller
public class ConnIpController {
	static Logger logger = LoggerFactory.getLogger(ConnIpController.class);
	...
	// 리스트
	@RequestMapping(value="/wrks/sstm/mpg/connIpList.do")
	public String connIpList(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		return "wrks/sstm/mpg/connIpList";
	}

	@RequestMapping(value="/wrks/sstm/mpg/connIpListData.json")
	@ResponseBody
	public Map<String, Object> connIpListData(@ModelAttribute("loginVO") LoginVO loginVO, ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		args.put("sidx"       , sidx);
		args.put("sord"       , sord);
		List<Map<String, String>> list = connIpService.selectConnIpInfoList(args);
	...
	@RequestMapping(value="/wrks/sstm/mpg/connIpList/insert.json")
	@ResponseBody
	public Map<String, Object> insert(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		map.put("authLvl", request.getParameter("authLvl"));
		...
		int insertResult;
		try {
			insertResult = connIpService.insertConnIpInfo(map);
	...
	// 수정
	@SuppressWarnings("unused")
	@RequestMapping(value="/wrks/sstm/mpg/connIpList/update.json")
	@ResponseBody
	public Map<String, Object> update(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int updateResult;
		try {
			updateResult = connIpService.updateConnIpInfo(map);
			mapRet.put("session", 1);
			mapRet.put("msg", "저장하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
	...
	// 삭제
	@RequestMapping(value="/wrks/sstm/mpg/connIpList/delete.json")
	@ResponseBody
	public Map<String, Object> delete(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int result = connIpService.deleteConnIpInfo(map);
		Map<String, Object> mapRet = new HashMap<String, Object>();
	...
	// 선택삭제
	@SuppressWarnings("unused")
	@RequestMapping(value="/wrks/sstm/mpg/connIpList/deleteMulti.json")
	@ResponseBody
	public Map<String, Object> deleteMulti(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int ret;
		try {
			ret = connIpService.deleteConnIpInfoMulti(list);
			mapRet.put("session", 1);
			mapRet.put("msg", "삭제하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
