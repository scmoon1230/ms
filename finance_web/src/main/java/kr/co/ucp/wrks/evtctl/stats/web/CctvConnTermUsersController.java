/**
 *---------------------------------------------------------------------------------- 
 * @File Name    : CctvConnTermUsersController.java
 * @Description  : 긴급영상제공통계 CCTV접속현황(기간/사용자)
 * @author       : seungJun
 * @since        : 2017. 2. 22.
 * @version      : 1.0
 *----------------------------------------------------------------------------------
...
@Controller
public class CctvConnTermUsersController
{
	...
	// 재난구분별 CCTV 접속현황
	@RequestMapping(value="/wrks/evtctl/stats/cctvTermUser.do")
	public String view(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception
	{
		...
		return "/wrks/evtctl/stats/cctvTermUsers";
	}

	@RequestMapping(value="/wrks/evtctl/stats/cctvTermUser/list.json") 
	@ResponseBody
	public Map<String, Object> getList(ModelMap model ,HttpServletRequest request ,HttpServletResponse response ) throws Exception
	{
		...
		List<Map<String, String>> resultRows = cctvConnCrtStateService.getTearmUserList(args);
	...
	@RequestMapping(value="/wrks/evtctl/stats/cctvTermUser/list.excel") 
	public String getExcelList(ModelMap model ,HttpServletRequest request ,HttpServletResponse response ) throws Exception
	{
		...
		args.put("searchSysCd", request.getParameter("searchSysCd"));
		List<Map<String, String>> resultRows = cctvConnCrtStateService.getTearmUserExcel(args);

		String titleKey = "USER_NM|TOT_EVT_CNT|CONN_EVT_CNT|CCTV_CONN_CNT|PTZ_CONN_CNT";
		String titleHeader = "사용자명|발생이벤트총수|CCTV접속근거이벤트수|CCTV접속건수|CCTV제어건수";
		// 제목 및 검색조건 추가
		model.put("title", "CCTV접속현황(기간/사용자)");
		model.put("search", searchEvtNm + " 서비스 (조회기간 : " + searchStartDate + "_" + searchEndDate + ")");
		model.put("fileName", searchStartDate + "_" + searchEndDate + "_" + searchEvtNm + "_CCTV접속현황(기간/사용자)" + "(" + EgovDateUtil.getToday());
