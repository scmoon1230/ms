/**
 *---------------------------------------------------------------------------------- 
 * @File Name    : CctvConnMonTypeController.java
 * @Description  : 긴급영상제공통계 CCTV접속현황(월/유형)
 * @author       : seungJun
 * @since        : 2017. 2. 22.
 * @version      : 1.0
 *----------------------------------------------------------------------------------
...
@Controller
public class CctvConnMonTypeController
{
	...
	// 재난구분별 CCTV 접속현황
	@RequestMapping(value="/wrks/evtctl/stats/cctvMonType.do")
	public String view(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception
	{
		...
		return "/wrks/evtctl/stats/cctvMonType";
	}

	@RequestMapping(value="/wrks/evtctl/stats/cctvMonType/list.json") 
	@ResponseBody
	public Map<String, Object> getList(ModelMap model ,HttpServletRequest request ,HttpServletResponse response ) throws Exception
	{
		...
		List<Map<String, String>> resultRows = cctvConnCrtStateService.getMonthTypeList(args);
	...
	@RequestMapping(value="/wrks/evtctl/stats/cctvMonType/list.excel") 
