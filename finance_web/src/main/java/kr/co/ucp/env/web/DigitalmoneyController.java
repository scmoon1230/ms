...
@Controller
public class DigitalmoneyController {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());

	@Resource(name="digitalmoneyService")
	DigitalmoneyService digitalmoneyService;

	@Resource(name="moneyService")
	MoneyService moneyService;

	// 리스트
	@RequestMapping(value="/env/digitalmoney.do")
	public String view(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		return "env/digitalmoney";
	}

	// 조건검색
	@RequestMapping(value="/env/digitalmoney/list.json")
	@ResponseBody
	public Map<String, Object> selectDigitalmoney(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<Map<String, String>> list = digitalmoneyService.selectDigitalmoney(args);
	...
	@RequestMapping(value="/env/digitalmoney/insert.json")
	@ResponseBody
	public Map<String, Object> insertDigitalmoney(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		map.put("stanGb", EgovStringUtil.nullConvert(request.getParameter("stanGb")));
		map.put("moneyType", EgovStringUtil.nullConvert(request.getParameter("moneyType")));
		...
		int insertResult;
		try {
			insertResult = digitalmoneyService.insertDigitalmoney(map);
			mapRet.put("session", 1);
	...
	// 수정
	@SuppressWarnings("unused")
	@RequestMapping(value="/env/digitalmoney/update.json")
	@ResponseBody
	public Map<String, Object> updateDigitalmoney(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int updateResult;
		try {
			updateResult = digitalmoneyService.updateDigitalmoney(map);
			mapRet.put("session", 1);
			mapRet.put("msg", "저장하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
	...
	// 삭제
	@RequestMapping(value="/env/digitalmoney/delete.json")
	@ResponseBody
	public Map<String, Object> deleteDigitalmoney(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int result = digitalmoneyService.deleteDigitalmoney(map);
	...
	// 다중삭제
	@SuppressWarnings("unused")
	@RequestMapping(value="/env/digitalmoney/deleteMulti.json")
	@ResponseBody
	public Map<String, Object> deleteDigitalmoneyMulti(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		int ret;
		try {
			ret = digitalmoneyService.deleteDigitalmoneyMulti(list);
			mapRet.put("session", 1);
			mapRet.put("msg", "삭제하였습니다.");
		}
		catch (DataIntegrityViolationException e) {
